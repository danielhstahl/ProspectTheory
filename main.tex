...
This motivates a series of constrained least squares problem (one for each gamble \(\sigma_i\)):

\begin{equation}
\begin{aligned}
\min_q \quad \norm{q_i-p_i} \\
\textrm{s.t.}\quad u(v_i)^T q_i = u(x^*_i)\\
\quad \sum_j q_{i,j} = 1
\end{aligned}
\end{equation}

Where \(u(x^*_i)\) is the ``actual'' utility of the gamble and whose ordering of the outcomes under \(\mathbb{Q}\) across gambles aligns with the ordering of preferences. This problem is easily solvable via Lagrange multipliers.
...
Solving the Lagrange, a \(q\) is chosen as \(q(\omega)=[0.0477, 0.885, 0.067]\).
...
\begin{equation*}
\begin{aligned}
q(\omega)=[0.046, 0.001, 0.001, 0.000, \\
0.855, 0.012, 0.018, 0.000, \\
0.065, 0.001, 0.001, 0.000]
\end{aligned}
\end{equation*}
...
\begin{center}
	\begin{tabular}{c c}
		...
		Choice 1 Option 1 & 13.911 \\
		Choice 1 Option 2 & 13.900 \\
		Choice 2 Option 1 & 11.562 \\
		Choice 2 Option 2 & 11.566 \\
	\end{tabular}
\end{center}
